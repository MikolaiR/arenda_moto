...
@section('title', 'Изменить мотоцикл')

@section('content')
    <h1 class="text-2xl font-semibold text-moto-orange mb-6">Изменить мотоцикл</h1>

    <form method="POST" action="{{ route('motorcycles.update', $motorcycle) }}"
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $motorcycle->name) }}" required
        ...
        <div>
            ...
            <input type="number" name="year" value="{{ old('year', $motorcycle->year) }}" required
                class="w-full bg-neutral-800 border border-neutral-600 rounded px-3 py-2 text-moto-text focus:border-moto-orange focus:outline-none">
            @error('year')
                <p class="text-red-400 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" name="state_number" value="{{ old('state_number', $motorcycle->state_number) }}"
                class="w-full bg-neutral-800 border border-neutral-600 rounded px-3 py-2 text-moto-text focus:border-moto-orange focus:outline-none">
            @error('state_number')
                <p class="text-red-400 text-sm">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <textarea name="comment" rows="3"
                class="w-full bg-neutral-800 border border-neutral-600 rounded px-3 py-2 text-moto-text focus:border-moto-orange focus:outline-none">{{ old('comment', $motorcycle->comment) }}</textarea>
        </div>
